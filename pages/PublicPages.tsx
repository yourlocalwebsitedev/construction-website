...
const SectionEyebrow: React.FC<{
  ...
  <div
    className={`text-xs font-bold tracking-[0.2em] mb-3 ${light ? "text-gold" : "text-gold"} ${className || ""}`}
  >
    {children}
  </div>
);
...
export const HomePage: React.FC<PageProps> = ({ language, onBookClick }) => {
  ...
  useEffect(() => {
    let mounted = true;
    Promise.all([getProjects(), getReviews(), getServices()]).then(
      ([p, r, s]) => {
        if (!mounted) return;
    ...
    <div>
      ...
      <section className="relative bg-ink flex items-center px-4 overflow-hidden py-16 sm:py-24 md:min-h-[680px] md:py-0">
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center sm:py-16">
          <SectionEyebrow className="mb-5 sm:mb-3">
            {t.home.eyebrow}
          </SectionEyebrow>
      ...
      <section className="bg-cream py-8 px-4 border-b border-ink/5">
        <div className="max-w-6xl mx-auto grid grid-cols-3 gap-2 sm:gap-6">
          ...
          <TrustItem
            ...
            href="https://scottsquality.yourlocalwebsitedev.workers.dev"
          />
          <TrustItem
            icon={<Award size={26} />}
            title={t.home.trust.warranty}
            sub={
              COMPANY.warrantyYears
                ? `${COMPANY.warrantyYears}-Year`
                : language === Language.EN
                  ? "On qualifying work"
      ...
      <section className="bg-ink py-16 md:py-20 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h2 className="font-serif text-3xl md:text-4xl font-bold text-cream">
            {t.home.moreThanFinish.title}
          </h2>
          <p className="text-cream/70 mt-5 leading-relaxed max-w-2xl mx-auto">
            {t.home.moreThanFinish.body}
          </p>
            ...
            {t.home.moreThanFinish.diagram.map((step, i) => (
              <React.Fragment key={step}>
                ...
                {i < t.home.moreThanFinish.diagram.length - 1 && (
                  <ChevronRight
                    size={16}
                    className="text-gold shrink-0 rotate-90 sm:rotate-0"
                  />
                )}
              </React.Fragment>
            ))}
      ...
      <section className="py-16 md:py-24 px-4 max-w-7xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-10">
          ...
          <Link
            to="/before-after"
            className="hidden sm:inline-flex items-center text-gold font-bold text-sm mt-4 sm:mt-0 hover:underline"
          >
          ...
          {BEFORE_AFTER_ITEMS.slice(0, 3).map((item) => (
            <div key={item.id}>
              <BeforeAfterSlider
                ...
                onInteract={() =>
                  trackEvent("before_after_interaction", { itemId: item.id })
                }
              />
              <div className="mt-3">
                ...
                <p className="text-xs text-body/70 flex items-center gap-1 mt-1">
            ...
            {services.map((service) => (
              ...
              >
                ...
                <div className="p-3 sm:p-4">
                  <h3 className="font-serif font-bold text-ink text-sm sm:text-base leading-tight">
                    {service.title[language]}
                  </h3>
                  <p className="text-[11px] sm:text-xs text-body/70 mt-1 line-clamp-2 hidden sm:block">
          ...
          {projects.map((p) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                <p className="text-sm text-body/80 mt-2 line-clamp-2">
                  {p.description[language]}
                </p>
                <div className="flex items-center gap-4 mt-3 text-xs text-body/60">
                  <span className="flex items-center gap-1">
                    <ImageIcon size={12} /> {p.images.length} {t.home.photos}
                  </span>
                  {p.videoUrl && (
                    <span className="flex items-center gap-1">
                      <VideoIcon size={12} /> 1 {t.home.videos}
                    </span>
                  )}
                </div>
        ...
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-left">
            <WhyCard
              ...
              label={t.home.whyUs.items.quality}
            />
            <WhyCard
              icon={<Brush size={22} />}
              label={t.home.whyUs.items.prep}
            />
            <WhyCard
              icon={<Eye size={22} />}
              label={t.home.whyUs.items.detail}
            />
            <WhyCard
              icon={<MessageCircleMore size={22} />}
              label={t.home.whyUs.items.comm}
            />
            <WhyCard
              icon={<ClipboardCheck size={22} />}
              label={t.home.whyUs.items.clean}
            />
            <WhyCard
              icon={<Wand2 size={22} />}
              label={t.home.whyUs.items.results}
            />
          </div>
        </div>
      </section>
      ...
      <section className="bg-softcream py-16 md:py-24 px-4">
        <div className="max-w-7xl mx-auto">
                ...
                <div className="flex text-gold mb-4">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      ...
                      className={i < r.rating ? "text-gold" : "text-ink/20"}
                    />
                  ))}
                </div>
                <p className="text-body mb-6 italic text-lg leading-relaxed">
                  "{r.text[language]}"
                </p>
                <div>
                  <div className="font-bold text-ink">{r.author}</div>
                  <div className="text-xs text-body/50 flex items-center mt-1">
                    <CheckCircle size={10} className="mr-1 text-green-600" />{" "}
                    {t.home.verifiedClient} • {r.source || "Google"}
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="text-center mt-10">
            <Link
              to="/reviews"
              className="inline-flex items-center bg-ink text-cream hover:bg-charcoal font-bold text-sm px-6 py-3 rounded-md tracking-wide transition-colors"
            >
      ...
      <section className="py-16 md:py-24 px-4 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row gap-10 items-center">
          ...
          <div className="md:w-1/2">
            <SectionEyebrow>
              {language === Language.EN ? "ABOUT US" : "NOSOTROS"}
            </SectionEyebrow>
            <h2 className="font-serif text-3xl md:text-4xl font-bold text-ink mb-4">
              {TRANSLATIONS[language].about.title}
              <br />
              {TRANSLATIONS[language].about.titleLine2}
            </h2>
            <p className="text-body leading-relaxed mb-6">
              {TRANSLATIONS[language].about.storyText}
            </p>
            <Link
              to="/about"
              className="inline-flex items-center text-gold font-bold hover:underline"
        ...
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {SERVICE_AREAS.map((a, i) => (
              <span
                key={i}
                className="bg-white border border-gold/30 text-ink text-sm px-4 py-2 rounded-full flex items-center gap-2"
              >
      ...
      <section className="bg-ink py-16 md:py-24 px-4 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-cream/70 mt-5 text-lg">
...
const TrustItem: React.FC<{
  ...
  href?: string;
}> = ({ icon, title, sub, titleClassName, href }) => {
  const content = (
    <div className="flex flex-col items-center text-center gap-2 p-3">
      <div className="text-gold shrink-0">{icon}</div>
      <div>
        <div
          className={`font-bold text-ink leading-tight ${titleClassName || "text-xs sm:text-sm md:text-base"}`}
        >
          {title}
        </div>
...
const WhyCard: React.FC<{ icon: React.ReactNode; label: string }> = ({
  icon,
  label,
}) => (
...
export const ServicesPage: React.FC<PageProps> = ({
  ...
  const subServicesFor = (slug: string) =>
    services.filter((s) => s.parentSlug === slug);

  const ServiceCard: React.FC<{ service: Service; large?: boolean }> = ({
    service,
    large,
  }) => (
    <div className="bg-white rounded-2xl shadow-md overflow-hidden border border-ink/5 flex flex-col group hover:shadow-xl transition-shadow">
      ...
      <div className="p-5 sm:p-6 flex-grow flex flex-col">
        {service.assemblySummary && (
          <p className="text-[10px] font-bold tracking-wide text-gold uppercase mb-1">
            {service.assemblySummary[language]}
          </p>
        )}
        <h3
          className={`font-serif ${large ? "text-2xl" : "text-lg"} font-bold text-ink mb-2`}
        >
          {service.title[language]}
        </h3>
        <p className="text-body/80 text-sm mb-6 flex-grow">
          {service.shortDescription[language]}
        </p>
  ...
  return (
    <div>
      <div className="bg-ink py-14 md:py-20 px-4 text-center">
        <SectionEyebrow>{t.home.services.eyebrow}</SectionEyebrow>
        <h1 className="font-serif text-3xl md:text-5xl font-bold text-cream">
          {t.services.title}
        </h1>
      </div>
...
export const PortfolioPage: React.FC<PageProps> = ({ language }) => {
  ...
  const getCategoryLabel = (cat: string) => {
    ...
    if (cat === ServiceCategory.RESIDENTIAL)
      return t.portfolio.filterResidential;
      ...
      {tab === "beforeAfter" ? (
        <div className="max-w-7xl mx-auto py-12 px-4">
          <div className="flex flex-wrap justify-center gap-3 mb-12">
            {[
              "All",
              ...Array.from(new Set(BEFORE_AFTER_ITEMS.map((b) => b.category))),
            ].map((cat) => (
              <button
                key={cat}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {(baFilter === "All"
              ? BEFORE_AFTER_ITEMS
              : BEFORE_AFTER_ITEMS.filter((b) => b.category === baFilter)
            ).map((item) => (
              <div key={item.id}>
                <BeforeAfterSlider
                  beforeSrc={item.beforeImage}
        ...
        <div className="max-w-7xl mx-auto py-12 px-4">
          ...
          {!loading && filteredProjects.length === 0 && (
            <div className="text-center py-20">
              <p className="text-body/60 text-lg">
                {language === Language.EN
                  ? "No projects found."
          ...
          <div className="md:w-1/2 space-y-8">
            <div>
              <h2 className="font-serif text-3xl font-bold text-ink mb-4 flex items-center">
                <span className="w-12 h-1 bg-gold mr-4"></span>
                {t.storyTitle}
              </h2>
...
export const ContactPage: React.FC<PageProps> = ({ language, onBookClick }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto py-16 px-4">
      <h1 className="font-serif text-4xl font-bold mb-2 text-center text-ink">
        {t.title}
      </h1>
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-8">
              <div className="flex items-start">
                ...
                <div>
                  <div className="font-bold text-sm text-cream/50 uppercase mb-1">
                    {t.phone}
                  </div>
                  <a
                    href={getTelUrl(COMPANY.phone)}
                    onClick={() =>
                      trackEvent("call_click", { context: "contact_page" })
                    }
